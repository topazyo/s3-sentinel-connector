...
from src.core.s3_handler import RetryableError
...
class TestS3Handler:
    ...
    def test_list_objects_with_time_filter(self, s3_handler, mock_s3_bucket):
        ...
        objects = s3_handler.list_objects(
            mock_s3_bucket, prefix="logs/firewall", last_processed_time=cutoff_time
        )
    ...
    def test_download_object_compressed(self, s3_handler, mock_s3_bucket):
        ...
        s3.put_object(Bucket=mock_s3_bucket, Key=key, Body=compressed_content)
    ...
    @pytest.mark.parametrize(
        ...
        [("SlowDown", True), ("NoSuchBucket", False), ("AccessDenied", False)],
    )
