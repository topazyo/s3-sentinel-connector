...
class TestCircuitBreakerStateTransitions:
    ...
    async def test_open_to_half_open_after_recovery_timeout(self):
        ...
        config = CircuitBreakerConfig(
            failure_threshold=2, recovery_timeout=1, min_calls_before_open=1  # 1 second
        )
        breaker = CircuitBreaker("test-service", config)
    ...
    async def test_half_open_to_open_on_failure(self):
        ...
        config = CircuitBreakerConfig(
            failure_threshold=2, recovery_timeout=1, min_calls_before_open=1
        )
        breaker = CircuitBreaker("test-service", config)
...
class TestCircuitBreakerFailureDetection:
    ...
    async def test_timeout_counts_as_failure(self):
        ...
        config = CircuitBreakerConfig(
            failure_threshold=2, operation_timeout=0.1, min_calls_before_open=1  # 100ms
        )
        breaker = CircuitBreaker("test-service", config)
    ...
    async def test_min_calls_before_open_enforced(self):
        ...
        config = CircuitBreakerConfig(
            failure_threshold=2, min_calls_before_open=5  # Need 5 calls before opening
        )
        breaker = CircuitBreaker("test-service", config)
...
class TestCircuitBreakerOpenError:
    ...
    async def test_circuit_open_error_details(self):
        ...
        config = CircuitBreakerConfig(
            failure_threshold=2, recovery_timeout=30, min_calls_before_open=1
        )
...
class TestCircuitBreakerHalfOpenLimits:
    ...
    async def test_half_open_max_calls_enforced(self):
        ...
        tasks = [
            asyncio.create_task(breaker.call(slow_succeeding_func)) for _ in range(2)
        ]
...
class TestCircuitBreakerMetrics:
    ...
    async def test_metrics_state_transitions_recorded(self):
        ...
        transitions = [
            t for t in metrics["state_transitions"] if t["to_state"] == "closed"
        ]
...
class TestCircuitBreakerDecorator:
    ...
    async def test_decorator_protects_function(self):
        ...
        config = CircuitBreakerConfig(failure_threshold=2, min_calls_before_open=1)
        breaker = CircuitBreaker("test-service", config)
...
class TestCircuitBreakerEdgeCases:
    ...
    async def test_zero_failure_threshold_invalid(self):
        ...
        config = CircuitBreakerConfig(
            failure_threshold=1, min_calls_before_open=1  # Allow opening after 1 call
        )
        breaker = CircuitBreaker("test-service", config)
    ...
    async def test_concurrent_calls_tracked_correctly(self):
        ...
        tasks = [asyncio.create_task(breaker.call(func, 0.1)) for _ in range(10)]
