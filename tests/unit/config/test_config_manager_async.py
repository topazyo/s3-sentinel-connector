...
class TestConfigManagerAsyncFactory:
    ...
    async def test_create_factory_with_vault(self, tmp_path):
        ...
        with patch('src.config.config_manager.SecretClient') as mock_client_class, \
...
class TestConfigManagerAsyncGetSecret:
    ...
    async def test_get_secret_not_found(self, tmp_path):
        ...
        with patch('src.config.config_manager.SecretClient') as mock_client_class, \
             patch('src.config.config_manager.DefaultAzureCredential'):
            mock_client = AsyncMock()
            mock_client.get_secret = AsyncMock(side_effect=ResourceNotFoundError("Secret not found"))
            mock_client_class.return_value = mock_client
...
class TestConfigManagerAsyncSyncCompatibility:
    ...
    @pytest.mark.asyncio
    async def test_resolve_secret_reference_with_async_client(self, tmp_path, monkeypatch):
        ...
        with patch('src.config.config_manager.SecretClient') as mock_client_class, \
            ...
            resolved_value = config_manager._resolve_secret_reference('keyvault:aws-access-key')
            
            assert resolved_value == 'env-fallback-key'  # Falls back to env var in sync context
...
class TestConfigManagerEdgeCases:
    ...
    async def test_factory_init_failure_handled(self, tmp_path):
        ...
        with patch('src.config.config_manager.SecretClient') as mock_client_class, \
             patch('src.config.config_manager.DefaultAzureCredential', side_effect=Exception("Auth failed")):
    ...
    async def test_resolve_secret_reference_env_fallback(self, tmp_path, monkeypatch):
        ...
        with patch('src.config.config_manager.SecretClient') as mock_client_class, \
             patch('src.config.config_manager.DefaultAzureCredential'):
            mock_client = AsyncMock()
            mock_client.get_secret = AsyncMock(side_effect=Exception("Key Vault unavailable"))
            mock_client_class.return_value = mock_client
