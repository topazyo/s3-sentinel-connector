...
class TestTokenRevocation:
    ...
    def access_control(self):
        ...
        admin_role = Role(name="admin", permissions=["admin:write", "admin:read"])
        user_role = Role(name="user", permissions=["user:read"])
    ...
    def test_revoke_token_missing_jti(self, access_control):
        ...
        token_without_jti = jwt.encode(
            payload, "test_secret_32_chars_minimum_123", algorithm="HS256"
        )
    ...
    def test_revoke_all_user_tokens(self, access_control):
        ...
        access_control.revoke_all_user_tokens("admin", reason="Account compromise")
    ...
    def test_revoke_all_user_tokens_with_logging(self, access_control):
        ...
        with patch.object(access_control.logger, "warning") as mock_log:
            access_control.revoke_all_user_tokens("admin", reason="Security incident")
    ...
    def test_get_revocation_stats(self, access_control):
        ...
        assert (
            stats["users_with_revocations"] == 1
        )  # Only admin has user-level revocation
    ...
    def test_concurrent_token_revocation(self, access_control):
        ...
        tokens = [access_control.generate_token("admin", expiry=3600) for _ in range(5)]
