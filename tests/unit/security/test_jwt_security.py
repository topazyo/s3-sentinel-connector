...
class TestPlainTextJWTBlocking:
    ...
    def test_plain_text_jwt_allowed_in_development(self):
        ...
        assert manager.access_control.jwt_secret == 'dev-secret-key-12345678901234567890'
    ...
    def test_production_case_insensitive(self):
        ...
        assert 'Plain-text JWT secrets are not allowed in production' in str(exc_info.value)


class TestSecureJWTFormats:
    """Test secure JWT format acceptance."""
    
    @patch.dict(os.environ, {'APP_ENV': 'production', 'JWT_SECRET': 'secure-prod-secret-1234567890123456'}, clear=False)
    def test_env_var_format_accepted_in_production(self):
        ...
        manager = SecurityManager(config)
        assert manager.access_control is not None
        assert manager.access_control.jwt_secret == 'secure-prod-secret-1234567890123456'
    ...
    @patch.dict(os.environ, {'APP_ENV': 'production', 'MY_JWT': 'prod-jwt-secret-12345678901234567890'}, clear=False)
    def test_custom_env_var_name(self):
        """Phase 5: Custom environment variable names should work."""
        config = get_test_config('env:MY_JWT')
        
        manager = SecurityManager(config)
        assert manager.access_control.jwt_secret == 'prod-jwt-secret-12345678901234567890'
...
class TestJWTAlgorithmSecurity:
    ...
    def test_hs256_algorithm_enforced(self):
        ...
        from src.security.access_control import User, Role
...
class TestBackwardsCompatibility:
    ...
    @patch.dict(os.environ, {'APP_ENV': 'staging', 'JWT_SECRET': 'staging-secret-key-1234567890123456'}, clear=False)
    def test_staging_environment_secure_format(self):
        ...
        manager = SecurityManager(config)
        assert manager.access_control.jwt_secret == 'staging-secret-key-1234567890123456'
