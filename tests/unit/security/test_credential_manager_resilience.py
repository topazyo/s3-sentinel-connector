...
def create_credential_manager():
    ...
    with patch('src.security.credential_manager.SecretClient', return_value=mock_client):
...
class TestKeyVaultCircuitBreaker:
    ...
    async def test_get_credential_timeout_increments_failure_count(self):
        ...
        assert credential_manager._circuit_breaker.state == CircuitState.CLOSED  # Still closed after 1 failure
    ...
    async def test_get_credential_opens_circuit_after_threshold(self):
        ...
        mock_secret_client.get_secret.side_effect = ServiceRequestError("Key Vault unavailable")
...
class TestKeyVaultRetryableErrors:
    ...
    async def test_resource_not_found_not_retryable(self):
        ...
        mock_secret_client.get_secret.side_effect = ResourceNotFoundError("Credential not found")
