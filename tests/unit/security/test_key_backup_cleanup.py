...
class TestKeyBackupCleanup:
    ...
    def encryption_config(self):
        ...
        return EncryptionConfig(
            key_rotation_days=1,
            max_backup_age_days=7  # 7 days retention
        )
    ...
    def test_cleanup_removes_old_backups(self, temp_key_store, encryption_config):
        ...
        recent_backup_file = os.path.join(temp_key_store, f"current.key.{recent_timestamp}")
    ...
    def test_cleanup_keeps_all_recent_backups(self, temp_key_store, encryption_config):
        ...
        for backup_file in backup_files:
            assert os.path.exists(backup_file), f"Recent backup should be kept: {backup_file}"
    
    def test_cleanup_removes_multiple_old_backups(self, temp_key_store, encryption_config):
        ...
        for backup_file in old_backups:
            assert not os.path.exists(backup_file), f"Old backup should be removed: {backup_file}"
    
    def test_cleanup_boundary_exactly_at_cutoff(self, temp_key_store, encryption_config):
        ...
        boundary_backup = os.path.join(temp_key_store, f"current.key.{cutoff_timestamp}")
        ...
        assert os.path.exists(boundary_backup), "Backup exactly at cutoff should be kept"
    ...
    def test_cleanup_handles_malformed_backup_names(self, temp_key_store, encryption_config):
        ...
        for malformed_file in malformed_files:
            assert os.path.exists(malformed_file), "Malformed backup should be skipped, not removed"
...
class TestKeyBackupCleanupIntegration:
    ...
    def test_cleanup_runs_automatically_on_key_save(self, temp_key_store, encryption_config):
        ...
        assert not os.path.exists(old_backup), "Old backup should be auto-removed on save"
    ...
    def test_cleanup_runs_on_key_rotation(self, temp_key_store, encryption_config):
        ...
        new_key = manager._rotate_key(old_key)
        
        # Verify old backup removed during rotation
        assert not os.path.exists(old_backup), "Old backup should be removed during rotation"
...
class TestKeyBackupCleanupLogging:
    ...
    def test_cleanup_logs_removed_backups(self, temp_key_store, encryption_config, caplog):
    ...
    def test_cleanup_logs_malformed_names(self, temp_key_store, encryption_config, caplog):
    ...
    def test_cleanup_logs_errors_non_fatal(self, temp_key_store, encryption_config, caplog):
        ...
        if platform.system() != 'Windows':
            ...
            assert "Failed to remove backup" in caplog.text or "cleanup failed" in caplog.text
...
class TestKeyBackupCleanupConfiguration:
    ...
    def test_custom_retention_period(self, temp_key_store):
        ...
        # Verify removal
        assert not os.path.exists(old_backup), "31-day backup should be removed with 30-day retention"
    ...
    def test_zero_retention_keeps_nothing(self, temp_key_store):
        ...
        assert not os.path.exists(recent_backup), "Backup should be removed with 0-day retention"
