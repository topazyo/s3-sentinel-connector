...
class TestSentinelRouterCircuitBreakerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_circuit_breaker_opens_on_repeated_azure_failures(
        self, sentinel_router
    ):
        ...
        sentinel_router.logs_client.upload.side_effect = AzureError(
        ...
        logs = [
            {"timestamp": "2024-01-01T00:00:00Z", "message": f"test-log-{i}"}
            for i in range(10)
        ]
    ...
    async def test_circuit_breaker_half_open_recovery(self, sentinel_router):
        ...
        sentinel_router.logs_client.upload.side_effect = ServiceRequestError(
            "Network failure"
        )
    ...
    @pytest.mark.asyncio
    async def test_circuit_breaker_fails_back_to_open_in_half_open(
        self, sentinel_router
    ):
    ...
    async def test_circuit_breaker_timeout_enforcement(self, sentinel_router):
        ...
        sentinel_router._circuit_breaker = CircuitBreaker(
            "azure-sentinel", short_timeout_config
        )
...
class TestCircuitBreakerFailedBatchIntegration:
    ...
    def sentinel_router_with_storage(self):
        ...
        router.blob_container_client = None
        router.local_failed_batch_dir = "failed_batches"
    ...
    @pytest.mark.asyncio
    async def test_failed_batches_stored_when_circuit_open(
        self, sentinel_router_with_storage, tmp_path
    ):
...
class TestCircuitBreakerObservability:
    ...
    async def test_circuit_breaker_transition_logged(self, sentinel_router, caplog):
        ...
        assert any(
            "closed" in record.message.lower() and "open" in record.message.lower()
