...
def mock_monitoring_components():
    """Mock monitoring component dependencies"""
    with patch('src.monitoring.PipelineMonitor') as mock_pipeline, \
         patch('src.monitoring.ComponentMetrics') as mock_metrics, \
         patch('src.monitoring.AlertManager') as mock_alerts:
        
        # Configure PipelineMonitor mock
        ...
        # Configure ComponentMetrics mock
        mock_metrics_instance = Mock()
        mock_metrics_instance.record_metric = Mock()
        mock_metrics_instance.get_metrics = Mock(return_value={
            'total_processed': 100,
            'total_errors': 2,
            'error_rate': 0.02,  # 2% error rate (healthy)
        ...
        mock_alerts_instance.check_alert_conditions = AsyncMock(return_value={
            'active_alerts': [],
            'resolved_alerts': [],
...
class TestGetComponentHealth:
    ...
    @pytest.mark.asyncio
    async def test_get_component_health_returns_healthy_status(self, mock_monitoring_config, mock_monitoring_components):
        ...
        manager.component_metrics['s3_handler'].get_metrics = Mock(return_value={
            'total_processed': 100,
            'total_errors': 2,
        ...
        # Assert
        assert health['status'] == 'healthy', "Component with 2% error rate should be healthy"
        assert 'metrics' in health, "Health response should include metrics"
        assert health['metrics']['error_rate'] == 0.02, "Metrics should include error_rate"
        assert 'last_check' in health, "Health response should include last_check timestamp"
    
    @pytest.mark.asyncio
    async def test_get_component_health_returns_degraded_status(self, mock_monitoring_config, mock_monitoring_components):
        """Test get_component_health returns degraded status when error rate >= 5%"""
        # Arrange
        manager = MonitoringManager(mock_monitoring_config)
        
        # Configure mock to return high error rate (15% - degraded)
        manager.component_metrics['sentinel_router'].get_metrics = Mock(return_value={
            'total_processed': 100,
            'total_errors': 15,
        ...
        # Assert
        assert health['status'] == 'degraded', "Component with 15% error rate should be degraded"
        assert health['metrics']['error_rate'] == 0.15, "Metrics should show high error rate"
    
    @pytest.mark.asyncio
    async def test_get_component_health_boundary_condition(self, mock_monitoring_config, mock_monitoring_components):
        ...
        assert health['status'] == 'degraded', "Component with exactly 5% error rate should be degraded"
    
    @pytest.mark.asyncio
    async def test_get_component_health_unknown_component(self, mock_monitoring_config, mock_monitoring_components, caplog):
        ...
        # Assert
        assert health['status'] == 'unknown', "Unknown component should return 'unknown' status"
    ...
    @pytest.mark.asyncio
    async def test_get_component_health_includes_timestamp(self, mock_monitoring_config, mock_monitoring_components):
    ...
    @pytest.mark.asyncio
    async def test_get_component_health_includes_all_metrics(self, mock_monitoring_config, mock_monitoring_components):
        ...
        assert health['metrics']['total_processed'] == 100, "Should include total_processed"
        ...
        assert health['metrics']['processing_time_avg'] == 0.15, "Should include processing_time_avg"
        assert health['metrics']['processing_time_p95'] == 0.25, "Should include processing_time_p95"
        assert health['metrics']['processing_time_p99'] == 0.35, "Should include processing_time_p99"
...
class TestCheckAlerts:
    ...
    @pytest.mark.asyncio
    async def test_check_alerts_returns_alert_status(self, mock_monitoring_config, mock_monitoring_components):
        """Test check_alerts returns alert status from AlertManager"""
        # Arrange
        manager = MonitoringManager(mock_monitoring_config)
        
        # Configure AlertManager mock to return alert status
        manager.alert_manager.check_alert_conditions = AsyncMock(return_value={
        ...
        # Assert
        assert 'active_alerts' in alert_status, "Alert status should include active_alerts"
        assert 'high_error_rate' in alert_status['active_alerts'], "Should include active alert"
        assert alert_status['total_alerts_checked'] == 2, "Should show number of alerts checked"
        
        # Verify AlertManager.check_alert_conditions was called
        manager.alert_manager.check_alert_conditions.assert_awaited_once()
    
    @pytest.mark.asyncio
    async def test_check_alerts_no_active_alerts(self, mock_monitoring_config, mock_monitoring_components):
        ...
        assert 'high_error_rate' in alert_status['resolved_alerts'], "Should show resolved alert"
    
    @pytest.mark.asyncio
    async def test_check_alerts_delegates_to_alert_manager(self, mock_monitoring_config, mock_monitoring_components):
        ...
        manager.alert_manager.check_alert_conditions = AsyncMock(return_value=expected_status)
        
        # Act
        alert_status = await manager.check_alerts()
        
        # Assert
        assert alert_status == expected_status, "Should return exact status from AlertManager"
        manager.alert_manager.check_alert_conditions.assert_awaited_once()
...
class TestCleanup:
    ...
    @pytest.mark.asyncio
    async def test_cleanup_cancels_all_tasks(self, mock_monitoring_config, mock_monitoring_components):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_waits_for_task_completion(self, mock_monitoring_config, mock_monitoring_components):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_handles_empty_tasks_list(self, mock_monitoring_config, mock_monitoring_components):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_handles_already_completed_tasks(self, mock_monitoring_config, mock_monitoring_components):
...
class TestMonitoringManagerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_monitoring_workflow(self, mock_monitoring_config, mock_monitoring_components):
        ...
        # Act - Full workflow
        await manager.start()
        await manager.record_metric('s3_handler', 'download_latency', 0.25, {'bucket': 'logs'})
        health = await manager.get_component_health('s3_handler')
        alerts = await manager.check_alerts()
        await manager.stop()
        
        # Assert
        assert health['status'] in ['healthy', 'degraded'], "Should have valid health status"
    ...
    @pytest.mark.asyncio
    async def test_health_check_after_multiple_metrics(self, mock_monitoring_config, mock_monitoring_components):
    ...
    @pytest.mark.asyncio
    async def test_alerts_after_degraded_health(self, mock_monitoring_config, mock_monitoring_components):
        ...
        # Assert
        assert health['status'] == 'degraded', "Component should be degraded with 20% error rate"
        assert 'high_error_rate' in alerts['active_alerts'], "Should have active alert for high error rate"
