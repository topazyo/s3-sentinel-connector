...
from src.monitoring import ComponentMetrics, PipelineMonitor
...
async def main():
    ...
    ComponentMetrics("sentinel_router")
    ...
    async def process_batch(batch):
        ...
        try:
            ...
            s3_metrics.record_processing(
                count=processed_count, duration=duration, batch_size=len(batch)
            )
            await monitor.record_metric(
                "logs_processed", processed_count, {"source": "s3", "status": "success"}
            )

        except Exception as e:
            s3_metrics.record_error(type(e).__name__)
            await monitor.record_metric(
                "logs_processed", 0, {"source": "s3", "status": "error"}
            )
            raise
