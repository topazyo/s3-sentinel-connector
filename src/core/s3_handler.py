...
class S3Handler:
    ...
    async def process_files_batch_async(
        ...
        if not objects:
            results['metrics'].update({'end_time': datetime.now(timezone.utc), 'duration': 0, 'success_rate': 0})
            return results

        chunks = [objects[i:i + self.batch_size] for i in range(0, len(objects), self.batch_size)]

        for chunk in chunks:
            tasks = []
            for obj in chunk:
                tasks.append(loop.run_in_executor(self._executor, self.download_object, bucket, obj['Key']))
            ...
            for obj, content in zip(chunk, downloaded):
                ...
                try:
                    ...
                    if parser:
                        parsed = await loop.run_in_executor(self._executor, parser.parse, content)
                        is_valid = await loop.run_in_executor(self._executor, parser.validate, parsed)
                        if not is_valid:
                            raise ValueError("Parsed content failed validation")
                        payload = parsed
                    else:
        ...
        results['metrics']['success_rate'] = success_count / results['metrics']['total_files'] if results['metrics']['total_files'] else 0
        return results
