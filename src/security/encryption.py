...
class EncryptionManager:
    ...
    def _initialize_keys(self) -> None:
        ...
        try:
            if not os.path.exists(self.key_store_path):
                ...
                self.logger.info(
                    f"Created key store with secure permissions: {self.key_store_path}"
                )
            ...
            self.logger.error(f"Failed to initialize encryption keys: {e!s}")
            raise
    ...
    def _cleanup_old_backups(self) -> None:
        ...
        try:
            cutoff_timestamp = int(time.time()) - (
                self.config.max_backup_age_days * 86400
            )
            removed_count = 0
                ...
                if ".key." in filename and filename.startswith("current.key."):
                    ...
                    try:
                        ...
                        if backup_timestamp < cutoff_timestamp:
                            ...
                            removed_count += 1
                            backup_age_days = (
                                int(time.time()) - backup_timestamp
                            ) // 86400
                            self.logger.info(
                        ...
                        self.logger.warning(
                            f"Skipping backup file with invalid timestamp: {filename} ({e!s})"
                        )
                        continue

                    except OSError as e:
                        # Log but don't abort cleanup for other files
                        self.logger.error(f"Failed to remove backup {filename}: {e!s}")
                        continue
            ...
            self.logger.error(f"Key backup cleanup failed (non-fatal): {e!s}")
    ...
    def _reencrypt_data(self, old_key: bytes, new_key: bytes) -> None:
        ...
        try:
            ...
            encrypted_files = [
                f for f in os.listdir(self.key_store_path) if f.endswith(".encrypted")
            ]
            ...
            for filename in encrypted_files:
                ...
                try:
                    ...
                    self.logger.error(f"Failed to re-encrypt {filename}: {e!s}")
            ...
            self.logger.error(f"Re-encryption process failed: {e!s}")
            raise
