...
class RotationManager:
    ...
    async def check_rotation_needed(self) -> List[str]:
        ...
        try:
            ...
            self.logger.error(f"Failed to check rotation status: {str(e)}")
            raise

    async def rotate_credentials(self, 
                               credential_names: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for credential_name in credential_names:
                try:
                    ...
                    new_value = await self.credential_manager.rotate_credential(
                        credential_name
                    )
                    ...
                    self.logger.error(
                        f"Failed to rotate credential {credential_name}: {str(e)}"
                    )
