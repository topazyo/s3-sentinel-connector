...
class AccessControl:
    ...
    def __init__(self, jwt_secret: str, min_secret_length: int = None) -> None:
        """
        ...
        self._user_revocation_timestamps: Dict[str, datetime] = {}  # username -> revocation time
    ...
    def validate_token(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if token_id and self.is_token_revoked(token_id):
                self.logger.warning(
                    "Revoked token used",
                    extra={'username': username, 'jti': token_id}
                )
    ...
    def revoke_token(self, token: str, reason: str = None) -> None:
        """
    ...
    def revoke_all_user_tokens(self, username: str, reason: str = None) -> int:
        """
        ...
        revoked_count = sum(
            1 for jti in self._revoked_tokens
    ...
    def cleanup_expired_revocations(self) -> int:
        ...
        expired_jtis = [
            jti for jti, expiry in self._revoked_token_expiry.items()
            if expiry < now
        ]
