...
class PermissionEnforcer:
    ...
    OPERATION_PERMISSIONS: ClassVar[Dict[str, str]] = {
    ...
    def enforce_permissions(
        ...
        if config_manager and hasattr(config_manager, "get_secret"):
            self._wrap_method(config_manager, "get_secret", "read:secrets")
            self.logger.info("Applied permission check to ConfigManager.get_secret")

        if encryption_manager:
            if hasattr(encryption_manager, "encrypt"):
                self._wrap_method(encryption_manager, "encrypt", "manage:encryption")
                self.logger.info(
                    "Applied permission check to EncryptionManager.encrypt"
                )

            if hasattr(encryption_manager, "decrypt"):
                self._wrap_method(encryption_manager, "decrypt", "manage:encryption")
                self.logger.info(
                    "Applied permission check to EncryptionManager.decrypt"
                )

        if s3_handler and hasattr(s3_handler, "delete_object"):
            self._wrap_method(s3_handler, "delete_object", "delete:objects")
        ...
        if credential_manager and hasattr(credential_manager, "rotate_credential"):
            self._wrap_method(
                credential_manager, "rotate_credential", "manage:credentials"
            )
            self.logger.info(
                "Applied permission check to CredentialManager.rotate_credential"
            )
