...
class RateLimiter:
    ...
    **Phase 6 (Performance):** Configurable to balance throughput vs protection
    ...
    def _inspect_and_consume(self, tokens: float):
        """
        Inspect current tokens, refill, and consume if available.

        Returns a tuple (acquired: bool, wait_time: float). If acquired is True,
        tokens have already been decremented. If False, wait_time is the number
        of seconds to wait before tokens will be available at the current rate.
        This method is synchronous and must be called while holding no external
        lock; it will acquire `self._lock` internally to ensure cross-path
        consistency between sync and async callers.
        """
        with self._lock:
            self._refill_tokens()
    ...
    def acquire(self, tokens: float = 1.0, timeout: Optional[float] = None) -> bool:
        ...
        **Phase 6 (Performance):** Thread-safe with minimal lock contention
        ...
        while True:
            acquired, wait_time = self._inspect_and_consume(tokens)
            ...
            if deadline and time.monotonic() >= deadline:
                logger.warning("Rate limit acquisition timeout after %.2fs", timeout)
                return False
    ...
    async def acquire_async(
        ...
        **Phase 6 (Performance):** Allows concurrent operations while waiting
        ...
        while True:
            # Use run_in_executor to call the shared synchronous path so that
            # sync and async acquisitions coordinate via the same lock.
            acquired, wait_time = await loop.run_in_executor(
                None, self._inspect_and_consume, tokens
            )
    ...
    def reset(self) -> None:
        ...
        **Phase 7 (Testing):** Useful for deterministic test setup
        ...
        with self._lock:
            ...
            self.last_update = time.monotonic()
            logger.info(
                "RateLimiter reset to full capacity (%.2f tokens)", self.capacity
            )
