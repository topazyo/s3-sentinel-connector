...
class PipelineMonitor:
    def __init__(
        ...
        if self.enable_background_tasks:
            ...
            try:
                ...
                self.logger.error(
                    "Cannot create tasks: no event loop running. Call await monitor.start() instead."
                )
    ...
    def _initialize_clients(self, metrics_endpoint: str) -> None:
        ...
        try:
            ...
            self.metrics_client = MetricsIngestionClient(
                endpoint=metrics_endpoint, credential=credential
            )
            self.logger.info("Successfully initialized monitoring clients")
        except Exception as e:
            self.logger.critical(f"Failed to initialize monitoring clients: {e!s}")
            raise
    ...
    async def update_component_health(
        self, component: str, status: bool, details: Optional[Dict[str, Any]] = None
    ) -> None:
        """
        ...
        self.logger.info(
            f"Component {component} health status: {'healthy' if status else 'unhealthy'}"
        )
            ...
            while True:
                try:
                    ...
                    await self.update_component_health(
                        "s3", s3_health["status"], s3_health
                    )

                    # Check Sentinel connectivity
                    sentinel_health = await self._check_sentinel_health()
                    await self.update_component_health(
                        "sentinel", sentinel_health["status"], sentinel_health
                    )
                    ...
                    self.logger.info(
                        "Health check loop cancelled, shutting down gracefully"
                    )
                    raise  # Re-raise to properly propagate cancellation
                except Exception as e:
                    self.logger.error(f"Health check loop error: {e!s}")
        ...
        try:
            while True:
                try:
                    ...
                    # Graceful shutdown requested
                    self.logger.info(
                        "Alert check loop cancelled, shutting down gracefully"
                    )
                    raise  # Re-raise to properly propagate cancellation
                except Exception as e:
        ...
        try:
            while True:
                try:
                    ...
                    # Graceful shutdown requested
                    self.logger.info(
                        "Metrics export loop cancelled, shutting down gracefully"
                    )
                    raise  # Re-raise to properly propagate cancellation
                except Exception as e:
                    self.logger.error(f"Metrics export error: {e!s}")
                    await asyncio.sleep(5)
        except asyncio.CancelledError:
    ...
    async def _check_alert_condition(self, alert_config: AlertConfig) -> None:
        ...
        try:
            ...
            self.logger.error(f"Alert check error for {alert_config.name}: {e!s}")

    async def _trigger_alert(
        self, alert_config: AlertConfig, current_value: float
    ) -> None:
    ...
    async def _send_teams_alert(self, alert_data: Dict[str, Any]) -> None:
        ...
        if not teams_webhook:
            self.logger.warning(
                "Teams webhook not configured; skipping alert for %s",
                alert_data.get("name"),
            )
            return
        ...
        for attempt in range(max_retries):
            try:
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with post_ctx as response:
                        if response.status == 200:
                            ...
                            self.logger.debug(
                                f"Teams alert sent successfully for {alert_data.get('name')}"
                            )
                            return
                ...
                self.logger.warning(
                    f"Teams alert attempt {attempt + 1}/{max_retries} failed: {e!s}"
                )
                if attempt < max_retries - 1:
                    # Exponential backoff: 1s, 2s, 4s
                    backoff_delay = 1 * (2**attempt)
                ...
                self.logger.error(f"Teams alert failed with unexpected error: {e!s}")
                return
    ...
    async def _send_slack_alert(self, alert_data: Dict[str, Any]) -> None:
        ...
        if not webhook:
            self.logger.warning(
                "Slack webhook not configured; skipping alert for %s",
                alert_data.get("name"),
            )
            return
        ...
        for attempt in range(max_retries):
            try:
                async with aiohttp.ClientSession(
                    ...
                    async with post_ctx as resp:
                        if resp.status == 200:
                            # Success - exit retry loop
                            self.logger.debug(
                                f"Slack alert sent successfully for {alert_data.get('name')}"
                            )
                            return
                ...
                # Network error or timeout - retry
                self.logger.warning(
                    f"Slack alert attempt {attempt + 1}/{max_retries} failed: {e!s}"
                )
                if attempt < max_retries - 1:
                    # Exponential backoff: 1s, 2s, 4s
                    backoff_delay = 1 * (2**attempt)
                ...
                # Unexpected error - log and abort
                self.logger.error(f"Slack alert failed with unexpected error: {e!s}")
                return
    ...
    async def _check_s3_health(self) -> Dict[str, Any]:
        ...
        if not self.s3_health_url:
            return {
                ...
                "detail": "no s3 health url configured",
            }
    ...
    async def _check_sentinel_health(self) -> Dict[str, Any]:
        ...
        if not self.sentinel_health_url:
            return {
                "status": True,
                "checked_at": datetime.now(timezone.utc).isoformat(),
                "detail": "no sentinel health url configured",
            }
